...
func (s *Sqlite) DeleteStudentById(id int64) error {
	...
	if err != nil {
		return fmt.Errorf("execution error while deleting student: %w", err)
	}
...
func (s *Sqlite) UpdateStudentById(id int64, updatedStudent types.Student) error {
	...
	result, err := stmt.Exec(updatedStudent.Name, updatedStudent.Email, updatedStudent.Age, id)
	if err != nil {
		return fmt.Errorf("execution error while updating student: %w", err)
	}
